...
- New trim size `5.5x8.5` (half-letter, 5.5" × 8.5") — a compact pocket /
  coil-pad format. All geometry functions support it via the generic
  width/height math; no API changes.

## [0.1.1]

### Changed

- Published under the scoped name `@alankherron/kdp-cover-geometry`. The
  repository remains `kdp-cover-math` on GitHub for historical / discovery
  reasons.
